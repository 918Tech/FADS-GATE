...
def _validate_registration(*, asset_id: str, country: str, region: str, platform: str) -> tuple[str, str, str, str]:
    ...
    if len(country) != 2 or not country.isalpha():
        raise AssetAuthError("country must be a two-letter country code")
    ...
    return asset_id, country, region, platform


def issue_asset_token(*, asset_id: str, country: str, region: str, platform: str, now: int | None = None, ttl_seconds: int | None = None) -> tuple[str, AssetIdentity]:
    asset_id, country, region, platform = _validate_registration(
        asset_id=asset_id, country=country, region=region, platform=platform
    )
    issued_at = int(time.time() if now is None else now)
    ttl = int(ttl_seconds if ttl_seconds is not None else os.environ.get("FADS_TOKEN_TTL_SECONDS", DEFAULT_TOKEN_TTL_SECONDS))
    ...
    identity = AssetIdentity(asset_id, country, region, platform, issued_at, expires_at)
...
def verify_asset_token(token: str, *, now: int | None = None) -> AssetIdentity:
    ...
    if int(header.get("ver", 0)) != TOKEN_VERSION:
        raise AssetAuthError("unsupported asset token version")
    ...
    if issued_at > current + 300:
        raise AssetAuthError("asset token issued in the future")
    ...
    return AssetIdentity(asset_id, country, region, platform, issued_at, expires_at)
